...
Socket.prototype.onPacket = function (packet) {
  if ('open' === this.readyState) {
    ...
    debug(this.id + ': packet received with closed socket');
  }
};
...
Socket.prototype.maybeUpgrade = function (transport) {
  debug(this.id + ': might upgrade socket transport from "%s" to "%s"'
  ...
  self.upgradeTimeoutTimer = setTimeout(function () {
    debug(this.id + ': client did not complete upgrade - closing transport');
    cleanup();
  ...
  function onPacket (packet) {
    if ('ping' === packet.type && 'probe' === packet.data) {
      ...
      debug(this.id + ': got upgrade packet - upgrading');
      cleanup();
  ...
  function check () {
    if ('polling' === self.transport.name && self.transport.writable) {
      debug(this.id + ': writing a noop packet to polling for fast upgrade');
  ...
  function onError (err) {
    debug(this.id + ': client did not complete upgrade - %s', err);
    cleanup();
    transport.close();
    transport = null;
  }
...
Socket.prototype.clearTransport = function () {
  ...
  this.transport.on('error', function () {
    debug(this.id + ': error triggered by discarded transport');
  });
  ...
  function onDrain () {
    if (self.sentCallbackFn.length > 0) {
      ...
      if ('function' === typeof seqFn) {
        debug(this.id + ': executing send callback');
        seqFn(self.transport);
      } else if (Array.isArray(seqFn)) {
        debug(this.id + ': executing batch send callback');
...
Socket.prototype.sendPacket = function (type, data, options, callback) {
  ...
  if ('closing' !== this.readyState && 'closed' !== this.readyState) {
    debug(this.id + ': sending packet "%s" (%s)', type, data);
...
Socket.prototype.flush = function () {
  if ('closed' !== this.readyState &&
    ...
    debug(this.id + ': flushing buffer to transport');
